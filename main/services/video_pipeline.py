...
S3_UPLOAD_WORKERS = int(os.getenv("S3_UPLOAD_WORKERS", min(8, CPU * 2)))
...
def get_or_create_category(folder_name):
    return VideoCategory.objects.get_or_create(
...
def extract_keyframes_and_store(url, filename, key, video_obj):
    ...
    def score_frame(i, raw_file):
        ...
        return (calculate_frame_score(final_path), final_path, i)

    with ThreadPoolExecutor(max_workers=FRAME_SCORE_WORKERS) as executor:
        futures = [
            executor.submit(score_frame, i, f)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            VideoProcessingState.objects.update_or_create(
                video=video_obj,
                defaults=dict(status="processing"),
            )

        # ===== SMART FRAME CHECK =====
        frames_count = VideoFrame.objects.filter(video=video_obj).count()

        if frames_count < TOP_FRAMES_LIMIT:
        ...
        if video_obj:
            VideoProcessingState.objects.update_or_create(
                video=video_obj,
                defaults=dict(status="failed", last_error=str(e)),
            )
...
def scan_s3_and_queue_videos():
    ...
    try:
        ...
        existing = set(Video.objects.values_list("video_url", flat=True))
        ...
        for page in paginator.paginate(Bucket=env["BUCKET_NAME"]):
            for obj in page.get("Contents", []):
                ...
                if not key or not key.endswith(".mp4"):
                    continue
                ...
                if db_url not in existing or states.get(db_url) != "completed":
